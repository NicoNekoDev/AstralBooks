package ro.nicuch.astralbooks.listeners;
...
import ro.nicuch.astralbooks.CitizensBooksAPI;
import ro.nicuch.astralbooks.CitizensBooksPlugin;
import ro.nicuch.astralbooks.dist.Distribution;
import ro.nicuch.astralbooks.persistent.BlockToBookPair;
import ro.nicuch.astralbooks.persistent.BlocksToBooksPairs;
import ro.nicuch.astralbooks.persistent.BlocksToBooksPairsDataType;
            ...
            if (chunkPersistentDataContainer.has(PersistentKey.CHUNK_TAG.getKey(), PersistentDataType.TAG_CONTAINER_ARRAY)) {
                ...
                for (BlockToBookPair blockDataContainer : blocksDataContainers.getList()) {
                    ...
                    ItemStack leftBook = (leftBookJson == null || leftBookJson.isEmpty()) ?
                            null :
                            this.distribution.convertJsonToBook(CitizensBooksAPI.GSON.fromJson(leftBookJson, JsonObject.class));

                    String rightBookJson = blockDataContainer.rightBook();
                    ItemStack rightBook = (rightBookJson == null || rightBookJson.isEmpty()) ?
                            null :
                            this.distribution.convertJsonToBook(CitizensBooksAPI.GSON.fromJson(rightBookJson, JsonObject.class));
